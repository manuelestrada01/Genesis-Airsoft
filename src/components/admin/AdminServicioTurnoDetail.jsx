...
export default function AdminServicioTurnoDetail() {
  ...
  const handleItemChange = (index, field, value) => {
    setPlanillaData((prev) => {
      ...
      const item = { ...items[index], [field]: field === "cantidad" || field === "precioUnitario" ? Number(value) : value };
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 24, alignItems: "start" }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 20, flexWrap: "wrap" }}>
                  <button className="af-save-btn" onClick={() => handleSavePlanilla(false)} disabled={saving}>Guardar planilla</button>
                  ...
                  <button className="af-save-btn" style={{ background: "#c8f400", color: "#000" }} onClick={() => handleSavePlanilla(true)} disabled={saving}>✓ Enviar al cliente (Completar)</button>
                </div>
              </div>
...
function InfoCard({ title, children }) {
  return (
    <div style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 12, padding: "16px 20px" }}>
      <h3 style={{ color: "#c8f400", fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 12px" }}>
        {title}
      </h3>
      {children}
    </div>
